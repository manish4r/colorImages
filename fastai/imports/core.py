...
required_version = Version("0.1.19")

try:
    installed_version = Version(version("fastprogress"))
    if installed_version < required_version:
        raise RuntimeError(f"fastprogress>={required_version} is required, but version {installed_version} is installed.")
except PackageNotFoundError:
    raise ImportError("fastprogress is not installed. Please install it with `pip install fastprogress`.")
...
def have_min_pkg_version(package, version):
    ...
    try:
        ...
        return installed_version >= Version(version)
